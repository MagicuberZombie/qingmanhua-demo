...
public class KeywordsFlow extends FrameLayout implements ViewTreeObserver.OnGlobalLayoutListener {
     ...
     * 随机分配的颜色值
     */
    String[] mrandomColor = {"#71c6d5", "#ffbc85", "#ac8fe5", "#ff7d7d", "#76beff"};
    /**
    ...
    private GradientDrawable setShape(int color) {
    ...
    private boolean show() {
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                txt.setBackground(setShape(Color
                        .parseColor(mrandomColor[i])));
                txt.setPadding(15, 5, 15, 5);
                // 获取文本长度
